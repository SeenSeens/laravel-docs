...
Artisan là giao diện dòng lệnh được tích hợp sẵn trong Laravel. Artisan nằm ở thư mục gốc của ứng dụng dưới dạng tập lệnh `artisan` và cung cấp một số lệnh hữu ích có thể hỗ trợ bạn trong quá trình xây dựng ứng dụng. Để xem danh sách tất cả các lệnh Artisan có sẵn, bạn có thể sử dụng lệnh `list`:
